...
from models.SNN_temporal import spike_encoding
...
def train(model, device, data_loader, optimizer, criterion):
    ...
    for states, actions in data_loader:
        ...
        optimizer.zero_grad()
        spikes = spike_encoding(states, num_steps, device)
        logits = model(spikes)
...
def main():
    ...
    model_path = "raw_data/snn_temporal_cartpole.pt"
    torch.save(model.state_dict(), model_path)
    print(f"Saved SNN policy to: {model_path}")
    # Save model

    curves_path = "results/snn_temporal_training_curves.json"
